 ...
 */
export default function BuscadorDeProducto({ productos, locale }: { productos: Producto[]; locale: Locale }) {
  ...
  return (
    <section className="rounded-3xl border border-line-dim bg-ink-2 p-6 sm:p-8">
      ...
      <p className="mt-2 text-sm text-text-dim">
        {d["buscador.descripcion"]}
      </p>

      {resultado ? (
        <div className="mt-5">
          <p className="text-sm text-text-dim">
            {d["buscador.resultadoIntro"]}
          </p>
          <div className="mt-4 flex items-center gap-4 rounded-2xl border border-line bg-ink p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-text-dim">${resultado.precio.toFixed(2)} · {d["buscador.precioReferencial"]}</p>
            </div>
          </div>
